...
const CUSTOM_SCRIPT_NAMES: &[&str] =
    &["backup.sh", "cleanup.py", "monitor_load.pl", "sync_data.rb"];
const CUSTOM_SCRIPT_IDS: &[&str] = &["task_123", "job_abc", "process_xyz", "item_789"];
const CUSTOM_SCRIPT_ERRORS: &[&str] = &[
    "E_TIMEOUT",
    "E_DISK_FULL",
    "E_PERM_DENIED",
    "E_CONFIG_MISSING",
...
const DOCKERD_IMAGE_NAMES: &[&str] = &[
    "ubuntu:latest",
    "postgres:14-alpine",
    "nginx:1.21",
    "custom_app:v1.2.3",
];
const DOCKERD_ACTIONS: &[&str] = &[
    "start",
    "stop",
    "create",
    "destroy",
...
pub fn format_syslog_entry(entry: &SyslogEntry) -> String {
    ...
    format!(
        "{} {} {} {}",
        ts_str, entry.hostname, app_pid_str, entry.message
    )
}

#[allow(clippy::cognitive_complexity, clippy::too_many_lines)]
fn generate_message_for_app(
    app_name: &str,
    rng: &mut StdRng,
    kernel_uptime_secs: f64,
    current_wall_time: &DateTime<Utc>,
) -> String {
    match app_name {
        "CRON" => {
        ...
        "sshd" => {
            ...
            match action_type {
                0 => format!(
                    ...
                    ip,
                    port,
                    SSH_RSA_FINGERPRINTS
                        .choose(rng)
                        .unwrap_or(&"key_fingerprint")
                ),
                1 => format!(
                ...
                2 => format!(
                    "Received disconnect from {} port {}:11: disconnected by user",
                    ip, port
                ),
                3 => format!(
                    "Failed password for {} user {} from {} port {} ssh2",
                    if rng.random_bool(0.5) { "invalid" } else { "" },
                    user,
                    ip,
                    port
                ),
        ...
        "kernel" => {
            let uptime_micros =
                (kernel_uptime_secs * 1_000_000.0) as u64 + rng.random_range(0..1_000_000);
            ...
            let mut message = KERNEL_MSG_TEMPLATES
                .choose(rng)
                .unwrap_or(&"generic kernel message")
                .to_string();
            ...
            message = message.replace(
                "{usb_driver}",
                USB_DRIVER.choose(rng).unwrap_or(&"xhci_hcd"),
            );
            ...
            message = message.replace(
                ...
                &format!(
                    "{}.{}",
                    current_wall_time.timestamp(),
                    current_wall_time.timestamp_subsec_micros()
                ),
            );
            message = message.replace(
                "{status}",
                if *(*["DENIED", "ALLOWED"].choose(rng).unwrap_or(&"DENIED")) == *"DENIED" {
                    "DENIED"
                } else {
                    "ALLOWED"
                },
            ...
            message = message.replace(
                "{ata_cmd}",
                ATA_CMD.choose(rng).unwrap_or(&"READ FPDMA QUEUED"),
            );
        ...
        "ntpd" => {
            ...
            match *action_ref {
                "synchronized to" => {
                    ...
                    format!(
                        "{} NTP server ({}) at stratum {}",
                        action_ref, server_ip, stratum
                    )
                }
        ...
        "postfix" => {
            ...
            match sub_type {
                ...
                _ => {
                    ...
                    let (d1, d2, d3, d4) = (
                        delay * rng.random_range(0.0..0.2),
                        delay * rng.random_range(0.1..0.3),
                        delay * rng.random_range(0.2..0.5),
                        delay * rng.random_range(0.3..0.7),
                    );
                    ...
                    let reason = if *status_val_ref != "sent" {
                        format!(
                            "(host {} said: {} {} - some_error_code)",
                            relay_srv,
                            rng.random_range(400..599),
                            POSTFIX_EMAILS.choose(rng).unwrap_or(&"unknown")
                        )
                    } else {
        ...
        "custom_script" => {
            ...
            let level_ref = ["INFO", "WARN", "DEBUG", "ERROR"]
                .choose(rng)
                .unwrap_or(&"INFO");
            ...
            match *level_ref {
                "INFO" => format!(
                    "{}: [{}] Processing item {}.",
                    level_ref, script_name, item_id
                ),
                "WARN" => format!(
                    "{}: [{}] Item {} has a minor issue.",
                    level_ref, script_name, item_id
                ),
                "DEBUG" => {
                    ...
                    format!(
                        "{}: [{}] Value of {} set to {}.",
                        level_ref, script_name, param, value
                    )
                }
                _ => {
                    ...
                    format!(
                        ...
                        level_ref, script_name, item_id, error
                    )
                }
            }
        }
        "samba" => {
            let samba_ts = current_wall_time
        ...
        "nfsd" => {
            if rng.random_bool(0.3) {
                ...
                let reason = *["access denied by server", "no_subtree_check", "sync error"]
                    .choose(rng)
                    .unwrap_or(&"unknown reason");
                format!(
                    "mountd: refused mount request from {} for /exports/data ({})",
                    client_ip, reason
                )
            } else {
                let auth_flavor = *["AUTH_NULL", "AUTH_SYS", "RPCSEC_GSS"]
                    .choose(rng)
                    .unwrap_or(&"AUTH_SYS");
        ...
        "kubelet" => {
            ...
            let message_body = match *event_type_ref {
                "SyncLoop" => format!(
                    ...
                    namespace,
                    pod_name,
                    KUBELET_STATUS.choose(rng).unwrap_or(&"running")
                ),
                "PLEG" => format!(
                    "\"{}\" Error: {}, Details: {}",
                    event_type_ref,
                    (*["FailedToGetContainerManager", "PodStopped"]
                        .choose(rng)
                        .unwrap_or(&"Error")),
                    (*["rpc error: code = DeadlineExceeded", "container not found"]
                        .choose(rng)
                        .unwrap_or(&"details"))
                ),
                "Probe" => format!(
                    ...
                    namespace,
                    pod_name,
                    KUBELET_PROBE_TYPES.choose(rng).unwrap_or(&"Liveness"),
                    KUBELET_PROBE_RESULTS.choose(rng).unwrap_or(&"Success")
                ),
                _ => format!(
                    "\"{}\" pod=\"{}/{}\" message=\"{}\"",
                    event_type_ref,
                    namespace,
                    pod_name,
                    (*["Configuration changed", "Pod sandbox changed"]
                        .choose(rng)
                        .unwrap_or(&"message"))
                ),
            };
...
pub fn generate_syslog_messages(count: usize, seed: u64) -> Vec<String> {
    ...
    for _ in 0..count {
        ...
        current_time +=
            Duration::seconds(time_increment_secs) + Duration::nanoseconds(time_increment_nanos);

        current_kernel_uptime_secs +=
            rng.random_range(0.0..5.0) + (rng.random_range(0..1_000_000) as f64 / 1_000_000.0);
        ...
        let message = generate_message_for_app(
            &app_name,
            &mut rng,
            current_kernel_uptime_secs,
            &current_time,
