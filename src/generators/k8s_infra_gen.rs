...
use rand::{Rng, SeedableRng, rngs::StdRng}; // SliceRandom removed as IndexedRandom should cover .choose() on slices
...
impl K8sObjectRef {
    fn new(kind: Option<&str>, namespace: Option<&str>, name_prefix: Option<&str>, rng: &mut StdRng) -> Self {
        let actual_kind = kind.unwrap_or_else(|| K8S_KINDS.choose(rng).unwrap_or(&"Pod")).to_string();
        let actual_ns = namespace.unwrap_or_else(|| NAMESPACES.choose(rng).unwrap_or(&"default")).to_string();
...
const K8S_KINDS: &[&str] = &["Pod", "Node", "Service", "Deployment", "ReplicaSet", "PersistentVolumeClaim", "Endpoints", "ConfigMap", "Secret"];
const NAMESPACES: &[&str] = &["default", "kube-system", "production", "staging", "dev-team-a", "monitoring", "logging"];
const POD_NAMES: &[&str] = &["frontend-", "backend-", "worker-", "database-", "cache-", "job-runner-"];
const NODE_NAMES: &[&str] = &["node-prod-01", "node-worker-b7", "kind-control-plane", "dev-node-spot-instance"];
const CONTAINER_IMAGE_NAMES: &[&str] = &["nginx:1.21-alpine", "postgres:14.2", "custom-app:v2.3.1", "redis:6.0", "pause:3.5"];
const ERROR_MESSAGES: &[&str] = &[
    "operation cannot be fulfilled", "an error on the server", "forbidden", "not found", "timeout", "conflict",
    "insufficient resources", "no nodes available", "failed to pull image", "pod sync failed", "failed to update node status",
    "admission webhook denied request", "failed to apply resource", "leader election lost", "slow fdatasync",
    "failed to reach quorum", "error syncing X", "failed to ensure load balancer"
];
const USERS: &[&str] = &["system:kube-scheduler", "system:serviceaccount:kube-system:deployment-controller", "user-dev@example.com", "system:node:node-worker-b7"];
const GROUPS: &[&str] = &["system:nodes", "system:authenticated", "system:masters", "developers"];
const WEBHOOK_NAMES: &[&str] = &["validation.example.com", "mutation.internal.svc", "kyverno-policy-controller"];
const CONTROLLER_NAMES: &[&str] = &["DeploymentController", "ReplicaSetController", "NodeController", "ServiceAccountController", "EndpointSliceController"];
const KUBELET_FILES: &[&str] = &["kubelet.go", "pleg.go", "volume_manager.go", "prober.go", "server.go", "container_manager_linux.go", "oom_watcher_linux.go"];
const APISERVER_FILES: &[&str] = &["storage_rbac.go", "generic_apiserver.go", "authentication.go", "audit.go", "etcd_health.go"];
const SCHEDULER_FILES: &[&str] = &["scheduler.go", "core_scheduler.go", "priorities.go", "binder.go", "framework.go"];
const ETCD_FILES: &[&str] = &["server.go", "raft.go", "mvcc_db.go", "wal.go", "cluster.go"];
const CONTROLLERMANAGER_FILES: &[&str] = &["node_controller.go", "deployment_controller.go", "serviceaccount_controller.go", "endpoints_controller.go", "job_controller.go"];
const KUBEPROXY_FILES: &[&str] = &["proxier.go", "service.go", "endpoints.go", "iptables.go"];


fn generate_k8s_object(rng: &mut StdRng, kind: Option<&str>, namespace: Option<&str>, name_prefix: Option<&str>) -> K8sObjectRef {
    K8sObjectRef::new(kind, namespace, name_prefix, rng)
}

#[allow(clippy::cognitive_complexity, clippy::too_many_lines)]
fn generate_message_for_component(component_name: &str, level: LogLevel, rng: &mut StdRng) -> String {
    ...
    let groups_str = groups_vec.iter().map(|s| **s).collect::<Vec<&str>>().join(",");
        ...
        "kube-scheduler" => match level {
            ...
            LogLevel::Warning | LogLevel::Error | LogLevel::Fatal => {
                let messages = [
                    ...
                    format!("\"Error binding pod to node\" pod=\"{}\" node=\"{}\" err=\"{}\"", pod_obj.to_namespaced_name(), node_name, error_msg_snippet),
                ];
        ...
        "etcd" => match level {
            LogLevel::Info => {
                ...
                let messages = [
                    ...
                    format!("\"raft.node: {} elected leader {} at term {}\"", from_id, to_id, rng.random_range(10..100)),
        ...
        "kube-proxy" => match level {
            LogLevel::Info => {
                let messages = [
                    ...
                    format!("\"Adding new service\" service=\"{}/{}\" ip=\"10.0.1.2\" port=\"80\"", pod_obj.namespace, pod_obj.name),
                    format!("\"Updating service\" service=\"{}/{}\"", pod_obj.namespace, pod_obj.name),
                ];
                messages.choose(rng).unwrap_or(&"Default KubeProxy Info".to_string()).to_string()
            }
            LogLevel::Warning | LogLevel::Error | LogLevel::Fatal => {
                let messages = [
                    format!("\"Error syncing iptables rules\" err=\"{}\"", error_msg_snippet),
                    format!("\"Failed to update service\" service=\"{}/{}\" err=\"{}\"", pod_obj.namespace, pod_obj.name, error_msg_snippet),
                    format!("\"Endpoint not found for service\" service=\"{}/{}\"", pod_obj.namespace, pod_obj.name),
                ];
                messages.choose(rng).unwrap_or(&"Default KubeProxy Error".to_string()).to_string()
            }
        },
        _ => format!("Generic message for {} - ID: {}", component_name, rng.random::<u32>()),
    }
}


pub fn format_klog_entry(level: LogLevel, timestamp: &chrono::DateTime<Utc>, thread_id: u32, file_line: &str, message: &str) -> String {
    format!("{} {} {:>7} {}:{}] {}",
        level,
...
pub fn generate_k8s_infra_logs(count: usize, seed: u64) -> Vec<String> {
    ...
    for _ in 0..count {
        ...
        let levels = [LogLevel::Info, LogLevel::Warning, LogLevel::Error, LogLevel::Fatal];
        ...
        logs.push(format_klog_entry(log_level, &current_time, thread_id, &file_line, &message));
    }
    logs
}
