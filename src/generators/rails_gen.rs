...
fn generate_rails_request_logs(rng: &mut StdRng, base_time: &mut chrono::DateTime<Utc>) -> Vec<String> {
    ...
    let method = HTTP_METHODS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "GET".to_string());
    let path_template = PATHS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "/users".to_string());
    let path = path_template.replace(":id", &rng.random_range(1..1000).to_string());
    let ip_address = format!("192.168.{}.{}", rng.random_range(0..256), rng.random_range(1..255));
    let controller = CONTROLLERS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "UsersController".to_string());
    let action = ACTIONS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "index".to_string());
    ...
    *base_time += Duration::microseconds(initial_jitter_micros);
    ...
    logs.push(format_log_line(*base_time, Severity::INFO, &context.request_id, context.pid, 
        &format!("Started {} \"{}\" for {} at {}", context.method, context.path, context.ip_address, base_time.format("%Y-%m-%d %H:%M:%S %z"))
    ));
    ...
    *base_time += Duration::microseconds(rng.random_range(100..5000));
    ...
    if !is_error_request {
        ...
        *base_time += Duration::microseconds(rng.random_range(500..10_000));
        ...
        let chosen_error = ERROR_DESCRIPTIONS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "NoMethodError: undefined method `foo' for nil:NilClass".to_string());
        *base_time += Duration::microseconds(rng.random_range(100..3000));
        logs.push(format_log_line(*base_time, Severity::FATAL, &context.request_id, context.pid, 
        ...
        for _ in 0..num_backtrace_lines {
            *base_time += Duration::microseconds(rng.random_range(50..1500));
            let file = BACKTRACE_FILES.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "app/controllers/application_controller.rb".to_string());
            let line_num = rng.random_range(10..200);
            let method_name = BACKTRACE_METHODS.choose(rng).map(|s| s.to_string()).unwrap_or_else(|| "unknown_method".to_string());
            logs.push(format_log_line(*base_time, Severity::FATAL, &context.request_id, context.pid, 
